...
"""
UniTouch-style ViT Projector training script (integrated with ID alignment).

Highlights / new features:
- **ID alignment (align_by_ids)**: before creating Dataset, compute intersection of tac_indices / rgb_indices
  and re-order so positive samples are true one-to-one ID matches.
- **No-replacement multi-sensor sampling** (sigma=0.75).
- **Optimizer grouping**: no weight decay for LayerNorm/bias/pos_emb/sensor_tokens/tau.
- **AMP**, early stopping, validation retrieval metrics (R@K / MedR / MeanR / mAP).
- **UniTouch default hyperparams**: dim=1024, depth=24, heads=16, τ=0.07 fixed, lr=1e-5, epochs=150.

Usage: run this file directly; edit the numpy paths in __main__ as needed.
"""
...
def align_by_ids(
    ...
    """Align tactile and image embeddings by ID intersection.
    Returns: tac_aln, rgb_aln, sensor_aln, kept_t_idx, kept_r_idx  (kept_* are indices into the original arrays)
    """
    ...
    keep.sort()  # sort by tactile order
    ...
    *,
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['R1_t2i'] > best_r1:
            ...
            print(f"* Saved new best model to: {best_path} (R@1_t2i={best_r1:.3f})")
        else:
            epochs_no_improve += 1
            if epochs_no_improve >= early_stop_patience:
                print(f"Early stopping: no improvement in val R@1 for {early_stop_patience} consecutive epochs.")
                break
    ...
    if sensor_ids is not None:
        ...
        if protos is not None:
            ...
            print("Saved sensor prototypes: ", save_path.replace('.pt', '_sensor_prototypes.npy'))

    return model


if __name__ == "__main__":
    # ====== Your data paths (modify as needed) ======
    tac_path = "embeddings/embeddings_tac/all_embeddings.npy"      # [N, D]
    rgb_path = "embeddings/embeddings_rgb/all_embeddings.npy"      # [N, D]
    tac_idx_path = "embeddings/embeddings_tac/all_indices.npy"     # [N] tactile IDs (optional, strongly recommended)
    rgb_idx_path = "embeddings/embeddings_rgb/all_indices.npy"     # [N] image IDs (optional, strongly recommended)
    sid_path = "embeddings/sensor_ids.npy"                          # [N] sensor IDs (optional)
    ...
    try:
        ...
        sensor_ids = None
        print("sensor_ids.npy not found; training without sensor annotations (no sensor batching or tokens).")

    # ====== NEW: ID alignment before training ======
    if tac_indices is not None and rgb_indices is not None:
        print("\n[Align] Index files detected, starting intersection-based alignment...")
        tac_aln, rgb_aln, sid_aln, kept_t_idx, kept_r_idx = align_by_ids(
            tac, tac_indices, rgb, rgb_indices, sensor_ids
        )
        print(f"[Align] tac original: {tac.shape[0]}, rgb original: {rgb.shape[0]} -> aligned N: {tac_aln.shape[0]}")
        tac = tac_aln
        rgb = rgb_aln
        sensor_ids = sid_aln
        # Optional: save aligned indices for reproducibility
        np.save("aligned_kept_tac_idx.npy", kept_t_idx)
        np.save("aligned_kept_rgb_idx.npy", kept_r_idx)
        # After alignment, optionally write back aligned embeddings / sensor ids
        ...
        # Also save paired mapping as a single npz for evaluation scripts
        np.savez("aligned_pairs.npz",
                 kept_t_idx=kept_t_idx,
                 kept_r_idx=kept_r_idx)

        print("[Align] Saved kept indices to aligned_kept_*.npy")
    else:
        print("[Align] No index files provided; assuming tac and rgb are already in one-to-one order.")
