...
"""
Stage-2 finetune script for the user's UniTouch-aligned ViT projector.

- Loads best checkpoint from Stage-1 (resume_path)
- Uses a lower LR with a short cosine schedule (defaults: lr=3e-6, epochs=60, T_max=50)
- Keeps the same architecture & loss (pure InfoNCE, fixed tau=0.07 by default)
- No-replacement multi-sensor batch sampler (sigma=0.75)
- Early stopping on val R@1 (t2i), best checkpoint saved as <save_path.replace('.pt','_best.pt')>

Defaults in __main__:
  resume_path = 'tac_projector_vit4_best.pt'
  save_path   = 'tac_projector_vit5.pt'
"""
...
def finetune_projector(
    ...
    *,
    ...
    for epoch in range(epochs):
        ...
        # Optionally evaluate EMA model instead:
        ...
        if val_metrics['R1_t2i'] > best_r1:
            ...
            print(f"* Saved new best model: {best_path} (R@1_t2i={best_r1:.3f})")
        else:
            epochs_no_improve += 1
    ...
    # Save final best weights (using EMA model)
    torch.save({'state_dict': ema_model.state_dict(), 'config': cfg}, best_path)
    print(f"Stage-2 finetuning complete. Best model saved to: {best_path}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        sensor_ids = None
        print("sensor_ids.npy not found; finetuning without sensor annotations (no sensor-specific batching or sensor tokens).")

    # finetune_projector(
    #     tac_embs=tac,
    #     rgb_embs=rgb,
    #     sensor_ids=sensor_ids,
    #     resume_path='tac_projector_vit2_best.pt',   # ← your Stage-1 best weights
    #     save_path='tac_projector_vit3.pt',          # ← stage-2 output
    #     lr=3e-6,
    #     epochs=100,
    #     batch_size=48,
    #     sigma=0.75,
    ...
    finetune_projector(
        ...
        rgb_embs=rgb,
        sensor_ids=sensor_ids,
        resume_path='tac_projector_vit5_best.pt',  # load previous-stage best
        save_path='tac_projector_vit5p.pt',        # polished output (p = polish)
        lr=3e-7,  # smaller LR
        epochs=30,  # short runs
        batch_size=48,
        sigma=0.75,
        val_ratio=0.1,
        weight_decay=0.05,
        early_stop_patience=10,  # more aggressive early stopping
        use_amp=True,
    )
